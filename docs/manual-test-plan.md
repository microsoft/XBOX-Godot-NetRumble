...
The full-lobby preflight and search-failure wording (cases 3.32–3.36) are covered by
`tools/tests/join_failure_tests.gd` against fake search results; the live cases are **not
exercised**. What a real full lobby's search result reports has not been observed yet: record
its `member_count`/`max_member_count` with 3.32. For a quick repro, lower the Deathmatch
`player_count` in `assets/tuning/mode_deathmatch.tres` to 2 locally so three clients suffice —
the Lobby minimum is 2 — and **do not commit it**. 3.34 needs a deliberately failed search,
